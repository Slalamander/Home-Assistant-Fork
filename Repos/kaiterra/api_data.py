...
async def test_api_connection(hass, user_data) -> bool:
    _LOGGER.warning(f"Testing Kaiterra API {user_data}")
    test_api = KaiterraAPIClient(
    ...
    test_result = test_api._is_valid_sensor_id(user_data["device_id"])
    _LOGGER.warning(f"Kaiterra API result: {test_result}")
    return test_result
